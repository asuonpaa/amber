...
const std::string kImageAutogenName = "_image_autogen";
  ...
  if (object_type == "IMAGE" || object_type == "IMAGE_ARRAY") {
    ...
    if (token->IsIdentifier() && token->AsString() == "AS") {
      ...
      if (buffer_type == BindingType::kColor) {
        ...
        for (auto& buf : buffers) {
          Image* image = script_->GetImage(buf->GetName() + kImageAutogenName);
          if (!image)
            return Result("autogenerated image from buffer not found");
          // TODO: Remove the buf version
    ...
    if (buffer_type == BindingType::kUnknown ||
      ...
      if (token->IsIdentifier() && token->AsString() == "DESCRIPTOR_SET") {
        ...
        // TODO Ari: All this kind of duplicate code with IMAGE should be
        // somehow combined.
        auto binding = token->AsUint32();
        uint32_t base_mip_level = 0;
        bool is_image = false;
...
Result Parser::ParseBuffer() {
  ...
  // TODO Ari: Here we create an image with a matching name + postfix.
  std::unique_ptr<Image> image = MakeUnique<Image>();
  buffer->SetName(name);
  image->SetName(name + kImageAutogenName);
  image->AddBuffer(buffer.get());
  // Default framebuffer size hacked here
  image->SetWidth(250);
  image->SetHeight(250);
  ...
  if (cmd == "DATA_TYPE") {
    ...
    // TODO Ari: Remove buffer version later
    buffer->SetFormat(image->GetFormat());
    script_->RegisterFormat(std::move(fmt));
  ...
  r = script_->AddImage(std::move(image));
  if (!r.IsSuccess())
    return r;
