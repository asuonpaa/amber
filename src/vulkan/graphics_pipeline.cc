...
Result GraphicsPipeline::CreateRenderPass() {
  ...
  for (const auto* info : color_buffers_) {
    ...
    attachment_desc.back().format =
        device_->GetVkFormat(*info->image->GetFormat());
  ...
  if (depth_stencil_buffer_.image &&
      depth_stencil_buffer_.image->GetFormat()->IsFormatKnown()) {
    attachment_desc.push_back(kDefaultAttachmentDesc);
    attachment_desc.back().format =
        device_->GetVkFormat(*depth_stencil_buffer_.image->GetFormat());
    attachment_desc.back().initialLayout =
...
Result GraphicsPipeline::Clear() {
...

  {
    ...
    if (depth_stencil_buffer_.image &&
      ...
      if (depth_stencil_buffer_.image->GetFormat()->HasStencilComponent())
